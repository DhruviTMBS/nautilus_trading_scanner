...
ib_contracts = [
    IBContract(secType='STK', exchange='NSE', symbol='TATASTEEL', currency='INR'),
]

gateway = InteractiveBrokersGatewayConfig(
    start=False,
    username="IB_GATEWAY_OR_TWS_USERNAME",
    password="IB_GATEWAY_OR_TWS_PASSWORD",
    trading_mode="paper",
    read_only_api=True,
)
...
config_node = TradingNodeConfig(
    ...
    exec_clients={
        "IB": InteractiveBrokersExecClientConfig(
            ...
            ibg_port="IBG_PORT",
            ibg_client_id=0,
            account_id="IBG_ACCOUNT_ID",  # This must match with the IB Gateway/TWS node is connecting to
            gateway=gateway,
            instrument_provider=instrument_provider,
...
strat_config = EMACrossConfig(
    instrument_id="TATASTEEL.NS",
    bar_type="TATASTEEL.NS-15-MINUTE-BID-EXTERNAL",
    fast_ema_period=10,
    slow_ema_period=20,
    trade_size=1000000,
)
